...
function scoreRestaurant(place: { rating?: number; userRatingCount?: number }) {
  ...
  if (place.rating != null) {
    if (place.rating >= 4.5) { score += 8; tags.unshift("Highly rated"); }
    else if (place.rating <= 3.8) { score -= 6; }
  }
  ...
  const loadNearby = async () => {
    ...
    try {
      ...
      const items: Place[] = (data.places ?? []).map((p: any) => {
        ...
        const scored = scoreRestaurant({ rating: p.rating, userRatingCount: p.userRatingCount });
    ...
    <View style={styles.container}>
      {loading && places.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable style={({ pressed }) => [styles.card, pressed && styles.pressed]} onPress={() => openDetails(item)}>
              <View style={styles.row}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.name}>{item.name}</Text>
                  <Text style={styles.meta}>
                    {item.distanceMiles.toFixed(1)} mi • {item.rating ? `${item.rating.toFixed(1)}⭐` : "—"}
                  </Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: "#FFFFFF", borderRadius: 18, borderWidth: 1, borderColor: "#E4EFF1", padding: 14, marginBottom: 10 },
  row: { flexDirection: "row", alignItems: "center", gap: 12 },
  name: { fontSize: 16, fontWeight: "900", color: "#0B2A2F" },
  meta: { marginTop: 4, color: "#4A6468", fontWeight: "600" },

  scorePill: { width: 56, height: 38, borderRadius: 12, alignItems: "center", justifyContent: "center", borderWidth: 1 },
  ...
  tag: { backgroundColor: "#F7FCFD", borderWidth: 1, borderColor: "#E4EFF1", paddingVertical: 6, paddingHorizontal: 10, borderRadius: 999 },
